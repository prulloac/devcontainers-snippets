...
INCLUDE_CBT=${CBT:-"false"}
INCLUDE_PUBSUB_EMULATOR=${PUBSUBEMULATOR:-"false"}
INCLUDE_BIGTABLE_EMULATOR=${BIGTABLEEMULATOR:-"false"}
...
install_using_apt() {
    ...
    if [ "${INCLUDE_CBT}" = "true" ]; then
        if ! (apt-get install -yq google-cloud-cli-cbt); then
            rm -f /etc/apt/sources.list.d/google-cloud-sdk.list
            return 1
        fi
    fi

    if [ "${INCLUDE_PUBSUB_EMULATOR}" = "true" ]; then
        if ! (apt-get install -yq google-cloud-cli-pubsub-emulator); then
            rm -f /etc/apt/sources.list.d/google-cloud-sdk.list
            return 1
        fi
    fi

    if [ "${INCLUDE_BIGTABLE_EMULATOR}" = "true" ]; then
        if ! (apt-get install -yq google-cloud-cli-bigtable-emulator); then
            rm -f /etc/apt/sources.list.d/google-cloud-sdk.list
            return 1
        fi
    fi
